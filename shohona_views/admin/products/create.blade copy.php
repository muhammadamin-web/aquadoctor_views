<x-app-layout>
                        ...
                        <div class="mb-4">
                            ...
                            <div id="other-ingredients-wrapper">
                                <div class="flex items-center mb-2">
                                    ...
                                    <input type="text" name="other_ingredients_price[]" placeholder="Narxi" class="block w-full px-4 py-2 border dark:bg-gray-900 rounded">
                                </div>
                            </div>
    ...
    // Asl qiymatni saqlash funksiyasi
    function clearFormatting(num) {
        return num.replace(/,/g, '');
    }

    // Barcha raqamli inputlarga tinglovchi qo'shish
    function addNumberFormattingListeners(input) {
        input.addEventListener('input', function() {
            const cleanedValue = clearFormatting(this.value);
    ...
    // Tinglovchi qo'shish barcha kerakli inputlarga
    document.querySelectorAll('input[type="number"], input[name="price"], input[name="other_ingredients_price[]"]').forEach(input => {
        addNumberFormattingListeners(input);
    });

    // Formni yuborishdan oldin raqamlarni asl qiymatga qaytarish
    document.querySelector('form').addEventListener('submit', function(event) {
        document.querySelectorAll('input[type="number"], input[name="price"], input[name="other_ingredients_price[]"]').forEach(input => {
            input.value = clearFormatting(input.value);
        });
    });

    // Select ingredient dropdown functionality
    ...
    document.querySelectorAll('.ingredient-checkbox').forEach(checkbox => {
        checkbox.addEventListener('change', function() {
            ...
            if (this.checked) {
                ...
                addNumberFormattingListeners(selectedIngredientsWrapper.querySelector(`input[name="ingredient_counts[${this.value}]"]`));
            } else {
    ...
    document.getElementById('add-other-ingredient').addEventListener('click', function() {
        ...
        // Yangi qo'shilgan inputga tinglovchi qo'shish
        const newPriceInput = wrapper.querySelector('input[name="other_ingredients_price[]"]:last-child');
        addNumberFormattingListeners(newPriceInput);
    });
    ...
    document.getElementById('calculate-cost').addEventListener('click', function() {
        ...
        document.querySelectorAll('input[name="other_ingredients_price[]"]').forEach(input => {
            totalCost += parseFloat(clearFormatting(input.value)) || 0;
        });
